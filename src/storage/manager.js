...
class StorageManager {
  ...
  async initializeTables() {
    ...
    try {
      for (const query of queries) {
        await this.d1Storage.db.prepare(query).run();
      }
  ...
  async saveSettings(settings) {
    ...
    // 这里简化处理，存储单个设置
    const existing = await this.d1Storage.prepare(`SELECT * FROM ${this.settingsTable} LIMIT 1`).all();
  ...
  async loadSettings() {
    const settings = await this.d1Storage.prepare(`SELECT * FROM ${this.settingsTable} LIMIT 1`).all();
